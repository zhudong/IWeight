...
public class HomeActivity extends BaseActivity {
    ...
    public View setInitView() {
        ...
        /***********************************************************/
        final SysApplication application = (SysApplication) getApplication();
        rootView.findViewById(R.id.ivSet).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                AlertDialog alertDialog = new AlertDialog.Builder(HomeActivity.this)
                        .setTitle("选择打印模式")
                        .setCancelable(false)
                        .setItems(new String[]{"佳博2120TF", "美团打印机", "商通打印机", "香山打印机"}, new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                Toast.makeText(HomeActivity.this, "选择了第" + which + "个", Toast.LENGTH_SHORT).show();
                                application.setPrintIndex(which + 1);//设置打印机的模式

                                SharedPreferences sharedPreferences = PreferenceUtils.getSp(HomeActivity.this);
                                sharedPreferences.edit().putInt("printIndex",(which+1)).apply();
                            }
                        }).create();
                alertDialog.show();
